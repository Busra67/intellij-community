...
package org.jetbrains.icons.scale
 ...
 */
fun factor(factor: Float): FactorScale {
    return IconManager.units().factorScale(factor.toDouble())
}

/**
 * Factor scale will scale the layer, including the sublayers, by the given factor. The parent scale is also applied.
 */
fun factor(factor: Double): FactorScale {
  return IconManager.units().factorScale(factor)
}
...
fun fitArea(width: IconUnit, height: IconUnit, relative: Boolean = true): FitAreaScale {
    return IconManager.units().fitAreaScale(width, height, relative)
}
...
fun fillArea(width: IconUnit, height: IconUnit, relative: Boolean = true): FillAreaScale {
    return IconManager.units().fillAreaScale(width, height, relative)
}
