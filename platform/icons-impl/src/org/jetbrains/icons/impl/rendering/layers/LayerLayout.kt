...
import org.jetbrains.icons.impl.modifiers.asPixels
...
class CompoundDimensions(val width: CompoundSize, val height: CompoundSize) {
    fun asPixels(scaling: ScalingContext): Dimensions {
        return Dimensions(width.asPixels(scaling).coerceAtLeast(0), height.asPixels(scaling).coerceAtLeast(0))
    }
}
...
class CompoundSize(
    ...
    operator fun plus(other: CompoundSize): CompoundSize {
        return if (other.pickMaxSet.isNotEmpty()) CompoundSize(pixels, dp, nestedCompounds + other)
        else {
    ...
    operator fun times(other: Float): CompoundSize {
        return CompoundSize(pixels, dp, nestedCompounds, pickMaxSet, multiplier * other)
    }

  operator fun times(other: Double): CompoundSize {
    return CompoundSize(pixels, dp, nestedCompounds, pickMaxSet, multiplier * other.toFloat())
  }
...
fun List<CompoundSize>.maxCompoundSize(): CompoundSize {
    return CompoundSize(DefaultPixel(0), DefaultDisplayPoint(0.0), emptyList(), this)
}
