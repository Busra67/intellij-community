...
internal fun RescalableImageResource.composeBitmap(density: Float, scale: IconScale): ImageBitmapView {
    val cache = (this as? GPUImageResourceHolder)
    val cached = cache?.getOrGenerateBitmap(SingleBitmapCache::class) { SingleBitmapCache() }
    if (cached != null) {
        return cached.getOrPut(this, density, scale)
    } else {
        return scale(density, scale).composeBitmap().toView()
    }
}
...
private class SingleBitmapCache {
    ...
    fun getOrPut(image: RescalableImageResource, density: Float, scale: IconScale): ImageBitmapView {
        val last = lastBitmap
        val expectedDimensions = image.calculateExpectedDimensions(density, scale)
        if (last == null) {
            return createNewBitmap(image, density, scale, expectedDimensions)
        } else {
            if (last.composeBitmap.size == expectedDimensions) {
                return last.composeBitmap
            } else if (last.dimensions.canFit(expectedDimensions)) {
                last.bitmap.setPixelsFrom(image.scale(density, scale))
    ...
    private fun createNewBitmap(image: RescalableImageResource, density: Float, imageScale: IconScale, dimensions: Bounds): ImageBitmapView {
        val (skia, compose) = image.scale(density, imageScale).composeBitmapWithoutCaching()
